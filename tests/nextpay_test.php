...
/**
 * nextpay enrolment plugin tests.
 *
...
class enrol_nextpay_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['nextpay'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['nextpay']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('nextpay'));
        $plugin = enrol_get_plugin('nextpay');
        $this->assertInstanceOf('enrol_nextpay_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_nextpay', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $nextpayplugin = enrol_get_plugin('nextpay');

        // Just make sure the sync does not throw any errors when nothing to do.
        $nextpayplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_nextpay_plugin $nextpayplugin  */
        $nextpayplugin = enrol_get_plugin('nextpay');
        ...
        $nextpayplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $nextpayplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $nextpayplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $nextpayplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $nextpayplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $nextpayplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $nextpayplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $nextpayplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $nextpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $nextpayplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $nextpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $nextpayplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $nextpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
